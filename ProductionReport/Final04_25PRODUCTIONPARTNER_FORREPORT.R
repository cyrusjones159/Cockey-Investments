...
#PRODUCTION RUNS HAVE 10-20 STOCKS ALL WHICH PUBLISHED EARNINGS AND HAVE LARGE DIVIDENDS. DOGS BOOTED.

result2 <- sqldf("SELECT symbol, dividend_yield FROM mydf2 WHERE dividend_yield > 1 Order by dividend_yield DESC")
autoresult2 <- sqldf("SELECT symbol, dividend_yield FROM automydf2 WHERE dividend_yield > 1 Order by dividend_yield DESC")
indresult2 <- sqldf("SELECT symbol, dividend_yield FROM indmydf2 WHERE dividend_yield > 1 Order by dividend_yield DESC")
healthresult2 <- sqldf("SELECT symbol, dividend_yield FROM healthmydf2 WHERE dividend_yield > 1 Order by dividend_yield DESC")
mediaresult2 <- sqldf("SELECT symbol, dividend_yield FROM mediamydf2 WHERE dividend_yield > 1 Order by dividend_yield DESC")
reitsresult2 <- sqldf("SELECT symbol, dividend_yield FROM reitsmydf2 WHERE dividend_yield > 1 Order by dividend_yield DESC")
...
con <- dbConnect(SQLite(), "submissionSCRUBBED.stats542.sqlite")
# Write the combined dataframe
dbWriteTable(con, "allstocks", superdf, overwrite = TRUE)
dbWriteTable(con, "sectorssummaries", supersummaries, overwrite = TRUE)
dbGetQuery(con, "SELECT COUNT(*) FROM allstocks")
# Close connection
dbDisconnect(con)
...
# Define the full set of fields you want in every sector output
final_fields <- c(
  "Company",
  "Ticker",
  "sector",
...
result5       <- result4b %>% select(all_of(final_fields))
autoresult5   <- autoresult4b %>% select(all_of(final_fields))
healthresult5 <- healthresult4b %>% select(all_of(final_fields))
mediaresult5  <- mediaresult4b %>% select(all_of(final_fields))
indresult5    <- indresult4b %>% select(all_of(final_fields))
reitsresult5  <- reitsresult4b %>% select(all_of(final_fields))
...
write_xlsx(reitsresult5,  "reits.result.xlsx")

supersummaries <- bind_rows(
  result5,
  autoresult5,
  healthresult5,
  mediaresult5,
...
write_xlsx(supersummaries,  "allsectors.result.xlsx")
